...
class TweetDfExtractor:
    ...
    def find_statuses_count(self)->list:
        ...
        statuses_count = [x.get('statuses_count', 0) 
        for x in user]
    ...
    def find_sentiments(self, text)->list:
        text = [x.get('retweeted_status', {}) 
        for x in self.tweets_list]
        extended_tweet = [x.get('extended_tweet', {}) 
        for x in text]
        full_text = [x.get('full_text', '') 
        for x in extended_tweet]

        sentimentedText = [TextBlob(x) 
        for x in full_text]
        polarity = []
        subjectivity = []
    ...
    def find_followers_count(self)->list:
        followers_count = [x.get('user', {}).get('followers_count', 0) 
        for x in self.tweets_list]
        return followers_count

    def find_friends_count(self)->list:
        friends_count = [x.get('user', {}).get('friends_count', 0) 
        for x in self.tweets_list]
        return friends_count

    def is_sensitive(self)->list:
        is_sensitive = [x.get('possibly_sensitive', None) 
        for x in self.tweets_list]

        return is_sensitive

    def find_favourite_count(self)->list:
        fav_count = [x.get('retweeted_status', {}).get('favorite_count', 0) 
        for x in self.tweets_list]
        return fav_count
    ...
    def find_retweet_count(self)->list:
        ...
        for x in self.tweets_list]
        retweet_count = [x.get('retweet_count', None) 
    ...
    def find_hashtags(self)->list:
        ...
        hashtags = [x.get('hashtags', None) 
        for x in self.tweets_list]
        return hashtags

    def find_mentions(self)->list:
        # retun list of all None
        mentions = [x.get('mentions', None) 
        for x in self.tweets_list]
        return mentions
    ...
    def find_lang(self)->list:
        lang = [x.get('retweeted_status', {}).get('lang', None) 
        for x in self.tweets_list]
        return lang    
